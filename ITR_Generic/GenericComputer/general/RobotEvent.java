...
public class RobotEvent {
	...
	public String toStringSend(){
		int checksum = (int)(((int)command.getValue() + (int)index + (int)(value & 0x00FF) + (int)(value >> 8)) % 255);
		StringBuffer st = new StringBuffer("U,");
		st.append(Integer.toString(command.getValue(),16));
		st.append(",");
		st.append(Integer.toString(index, 16));
		st.append(",");
		st.append(Integer.toString(value,16));
		st.append(",");
		st.append(Integer.toString(checksum,16));
		st.append("\n");
		return st.toString();
	}
